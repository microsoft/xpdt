#!/bin/bash
#
echo "$0 $@" > betaLinuxPerformanceTool.log
(
#############################################################
#                   START Define vars     				    #
#############################################################
...
DIRNAME=betaLinuxPerformanceTool
...
if ! [[ $LIMIT =~ $RE ]]
	then
		echo -e " *** Usage: ./betaLinuxPerformanceTool.sh -ps <capture time in seconds>"
		exit 0
fi
}
...
if [[ $HITS == 0 || $WAIT == 0 ]]
	then
		...
		echo " *** Usage: ./betaLinuxPerformanceTool.sh -pl <nr. of samples> <interval in seconds>"
		exit 0
fi
...
sudo tar -rvf $PACKAGE_NAME betaLinuxPerformanceTool.log
}

append_pid_files () {

if [ -f /tmp/betaLinuxPerformanceTool_start-$DATE_START.pid ]
	then 
		cp /tmp/betaLinuxPerformanceTool_start-$DATE_START.pid .
		sudo tar -rvf $PACKAGE_NAME betaLinuxPerformanceTool_start-$DATE_START.pid
fi

if [ -f /tmp/betaLinuxPerformanceTool_stop-$DATE_STOP.pid ]
	then 
		cp /tmp/betaLinuxPerformanceTool_stop-$DATE_STOP.pid .
		sudo tar -rvf $PACKAGE_NAME betaLinuxPerformanceTool_stop-$DATE_STOP.pid
fi
}
...
rm -rf /tmp/betaLinuxPerformanceTool*
bash -c 'echo $PPID' > /tmp/betaLinuxPerformanceTool_start-$DATE_START.pid
}

get_pid_stop () {
DATE_STOP=$(date +%d.%m.%Y_%HH%MM%Ss)
cp /tmp/betaLinuxPerformanceTool_start-$DATE_START.pid /tmp/betaLinuxPerformanceTool_stop-$DATE_STOP.pid
}
...
calc () {
    ...
    hour_minute () {
        ...
        echo " *** For a $CAPTURE_PERIOD hours capture in $CAPTURE_INTERVAL minutes interval, this is your command: './betaLinuxPerformanceTool.sh -pl $PARAM1 $PARAM1_DWN'"
        echo " *** Use 'nohup ./betaLinuxPerformanceTool.sh -pl $PARAM1 $PARAM1_DWN &' to be able to disconnect your remote session and keep capture going"
    }
    ...
    minute_second () {
        ...
        echo " *** For a $CAPTURE_PERIOD minutes capture in $CAPTURE_INTERVAL seconds interval, this is your command: './betaLinuxPerformanceTool.sh -pl $PARAM1 $PARAM1_DWN'"
        echo " *** Use 'nohup ./betaLinuxPerformanceTool.sh -pl $PARAM1 $PARAM1_DWN &' to be able to disconnect your remote session and keep capture going"
    }
    ...
    hour_second () {
        ...
        echo " *** For a $CAPTURE_PERIOD hours capture in $CAPTURE_INTERVAL seconds interval, this is your command: './betaLinuxPerformanceTool.sh -pl $PARAM1 $PARAM1_DWN'"
        echo " *** Use 'nohup ./betaLinuxPerformanceTool.sh -pl $PARAM1 $PARAM1_DWN &' to be able to disconnect your remote session and keep capture going"
    }
...
header_linux () {
echo " ---------------- $(date) -----------------"
echo " ----------- Running betaLinuxPerformanceTool (v$SCRIPT_VERSION) -----------"
}
...
case $1 in
		...
		-h) 
			...
			echo "     Usage:./betaLinuxPerformanceTool.sh -ps <time to capture in seconds>, performance short-mode."
		    echo "	   ./betaLinuxPerformanceTool.sh -pl <nr. of samples> <sampling interval in seconds>, performance long-mode." 
			echo "                   Can  be used with 'nohup' and sent to background [&] in long run captures, when remote "
			echo "                   sessions need to be disconnected."
			echo "           ./betaLinuxPerformanceTool.sh -m, calculator for time parameters for '-pl' option."
			echo ""
...
# EOF
#
) 2>&1 | tee -a betaLinuxPerformanceTool.log
